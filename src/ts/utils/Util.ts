...
export default class Util {
    ...
    /**
     * 获取倒数计时的方法,
     * 参数1：endTime是结束时间，required:需要大于现在的时间，format:；type:字符串
     * ex:Utils.getLastTime('2019-01-29 15:30:00');
     * return:一个对象数据格式,format:{days:4,hours:4,minutes:4,seconds:4}
     */
    public static getLastTime(endTime:string) {
     ...
     * 初步判断用户是否登录
     * 参数1：none
     * ex:Utils.easyIsLogined();
     * return:true-用户已经登录，false-用户未登录
     */
    public static easyIsLogined(){
        let token= LocalStorageUtil.getUserToken().account_token||Util.getUrlParam("account_token");
