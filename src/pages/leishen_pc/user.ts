...
class User extends UserProxy {
    ...
    public isStayCurentTab:boolean=false;
    ...
    public getUserinfoSuccess() {
        ...
        if(!this.isStayCurentTab) {
            this.tabIndex = 0;
            this.isStayCurentTab = true;
        }
        this.userinfoCount++;
        //屏蔽老用户绑定账号提示
     ...
     * 刷新用户信息
     */
    public reGetUserInfo(stayCurentTab?:boolean) {
        if(!stayCurentTab){
            this.isStayCurentTab=stayCurentTab
        }else{
            this.isStayCurentTab=true
        }
