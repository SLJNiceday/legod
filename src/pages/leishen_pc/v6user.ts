...
import ResetUserinfo from './v6/ResetUserinfo.vue';
import UserSafety from './v6/UserSafety.vue';
import ThreeBind from './v6/Threebind.vue';
import BindMobile from './v6/BindMobile.vue';
import activeRecord from './v6/ActiveRecord.vue';
...
class User extends UserProxy {
    ...
    public serviceShow: boolean = true;//下拉框是否显示
    public shadowShow: boolean = false;//遮罩层是否显示
    public tabIndex: number = 0;//tab_nav 默认显示第一个
    public appParam: AppParamModel = AppParamModel.getInstace();
    public bgImg: string = './images/bz_bg.png';
    ...
    public goFirstTab:boolean=false; //初次加载(老用户刚刚绑定手机号)
    /**
    ...
    public created() {
        ...
        this.init();
        //如果是第三方登录绑定成功，并且重新加载用户中心
        if( localStorage.getItem('pageIndex')=='2'){
            this.goFirstTab=false
            this.tabIndex=2
     ...
     * 设置背景
     */
    public setBackground(imgurl: string) {
        this.bgImg = imgurl;
    }
    ...
    * 重新获取用户信息，并且保留在当前Tab
     */
    public reGetUserinfo(goFirstTab:boolean) {
        this.goFirstTab=goFirstTab
        this.getUserInfo()
    }
    /**
     * 下载最新版本
    ...
    /**
     * 刷新用户信息
     * stayCurentTab:是否停留在当前Tab,主要是活动页面用户领取雷神充值卡以后，要留在当前Tab,还要重新刷新用户时长
     */
    public reGetUserInfo(goFirstTab?:boolean) {
        if(goFirstTab){
            this.goFirstTab=goFirstTab
        ...
        if (this.backData.code == HttpClient.HTTP_SUCCESS_NET_CODE) {
            ...
            for (var i = 0; i < this.bannerList.length; i++) {
                ...
                if(imgUrls != null && imgUrls != undefined) {
                    this.bannerList[i]['imgUrl'] = imgUrls.img_url;
                }
                let start_time = new Date(this.bannerList[i].start_time).getTime();
                let end_time = new Date(this.bannerList[i].end_time).getTime();
                let now_time = new Date(this.bannerList[i].now_time).getTime();
                if(start_time < now_time && now_time  < end_time) {
                    this.bannerList[i]['is_show'] = true;
                }else {
                    this.bannerList[i]['is_show'] = false;
                }
            }
        }
    }
