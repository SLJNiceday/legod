<template>
    <div>
        <div class="mui-content orderBox" v-show="!isNeedUserAddress">
            ...
            <van-action-sheet v-model="prizeBoxShow" title="选择礼包类型">
                <p v-for="item in card_list"  @click="confirmGiftType(item)" style="border-top:1px solid #e2dcdc;line-height: 0.4rem;text-align: center;" :key="item.key">{{item.title}}</p>
            </van-action-sheet>
        </div>
                ...
                <van-field v-model="phone" style="margin-top:2px;" required placeholder="请输入">
                        <span slot="left-icon" @click="AreaCodeshow=true" style="cursor:pointer; ">
                            ...
                            <span style="line-height: 24px;vertical-align: text-bottom;">
                                <span style="margin-right:6px;"> 手机号</span>
                                <img :src="countryCode.ico">
