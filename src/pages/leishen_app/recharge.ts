...
class Recharge extends RechargeProxy {
    ...
    public async getDownloadUrl() {
        ...
        if(this.userInfo.is_pay_user == 0) {
            let data = jsonConfig.leigod.shouchong_banner;
     ...
     */
    setChannel() {
        let channel_from = Util.getUrlParam("from");
        if (channel_from) {
            LocalStorageUtil.setCookie(LocalStorageUtil.SRC_CHANNEL, channel_from, -1, true);
        }
    ...
    public async getUserInfo() {
        try {
            ...
            if (this.backData.code == HttpClient.HTTP_SUCCESS_NET_CODE) {
                let userInfo=this.backData.data
                if(userInfo.region_code==1&&userInfo.mobile==''){
                    Toast('检测你未绑定手机号请您首先绑定手机号，然后继续操作')
                    setTimeout(()=>{
    ...
    confirmZhekou(){
        if(this.discountList.length>0){
            ...
            for(let qq=0;qq<this.discountList.length;qq++){
               if(this.discountList[qq].title==this.zheCodeCopy){
                   findable=true
               }
            }
            if(!findable){
                //如果既有共有折扣码，又有私有折扣码,用户选择输入共有折扣码
                this.zheCode=this.zheCodeCopy
            }
        }else{
    ...
    public onChoosePrice(type: any = null) {
        ...
        if (this.discountList.length == 0) {
            ...
            this.zheCodeCopy=this.discountList[0].title
        }
    }
        ...
        if (this.backData.code == HttpClient.HTTP_SUCCESS_NET_CODE) {
            ...
            if (activityList.length > 0) {
                activityList.forEach(element => {
                    let a: { img?: string; url?: string,is_show?: boolean } = {};
                    a.img = this.imageHeadUrl + element.imgs.filter((a, b) => {
                        return a.key == 1; // 0 官网  1是移动端  2pc 客户端
                    })[0].img_url;
                    //判断是否是外链  是取m_url  不是链接给javaScript:;
                    if (element.url_type != 0) {
                        a.url = element.m_url;
                    }else {
                        a.url = 'javaScript:;';
                    }
                    let start_time = new Date(element.start_time.replace(/-/g,'/')).getTime();
                    let end_time = new Date(element.end_time.replace(/-/g,'/')).getTime();
                    let now_time = new Date(element.now_time.replace(/-/g,'/')).getTime();
                    if(start_time < now_time && now_time < end_time) {
                        a.is_show = true;
                    }else {
                        a.is_show = false;
