import "./css/wap.less";
import {Popup, Picker, Tab, Tabs, Toast, Icon, Loading, Checkbox, Field,CellGroup} from "vant";
...
class Login extends LoginProxy {
     ...
     */
    onLoginSuccess() {
        let user_info=LocalStorageUtil.getUserInfo();
        if(user_info.region_code==1&&user_info.mobile==''){
            this.isshowLogin=false;
            this.registerIsCaptcha();
            Toast('登录成功！检测你未绑定手机号请您首先绑定手机号，然后继续操作');
        }else{
    ...
    public gotoActives(activehtml) {
        ...
        let param:any='';
        if(refer_code){
             param = "platform=" + appParam.platform+'&refer_code='+refer_code;
        }else{
             param = "platform=" + appParam.platform;
