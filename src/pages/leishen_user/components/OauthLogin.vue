...
<template>
    <div class="flex_column_center" style="z-index: 999;">
        <p class="pos_warm_txt" style="color:#fff;">{{$t("public.share37")}}</p>
        <div class="mar_t10" v-if="ispopwindow">
        ...
        <ul class="flex_row_around" style="width: 240px;margin: 0 auto;" v-if="!ispopwindow">
...
<script lang="ts">
    ...
    export default class OauthLogin extends OauthProxy {
        @Prop(Boolean) ispopwindow!:Boolean;
        ...
        public setbindurltype(type:string) {
            if(this.ispopwindow){
                let offsetLeft=(document.body.clientWidth/2)+($('#userLoginRegisterFind .el-dialog').width()/2)
                switch (type) {
                    case 'qq':
