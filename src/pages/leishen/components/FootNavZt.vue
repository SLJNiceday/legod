<template>
    <div class="foot" id="llz_leishen_footer">
        <div class="footer_llz_newsBox" v-show="newsList.length!=0">
            <ul class="flex_row_center">
                <li v-for="item in newsList" @click="goToNews(item)" :key="item.id" class="footer_llznewsBoxLi">
                    <div class="llz_imageContianer">
                        <div :style="{background:'url('+picServerBase+item.image_url+')'}" class="footer_llz_newsImg">
                        </div>
                    </div>
<!--                    <img :src="picServerBase+item.image_url" width="280px" height="165px" style="border-radius: 4px;" :alt="item.title">-->
                    <p class="footer_llz_newsTitle" :title="item.title">{{item.title}}</p>
                    <p class="footer_llz_newsDesc" :title="item.summary">{{item.summary}}</p>
                </li>
            </ul>
        </div>
                    ...
                    <ul class="clear_fix">
                        ...
                        <li class="pos_foot_img f_left">
                            <a
                                ...
                                <img src="//static.anquan.org/static/outer/image/hy_124x47.png?id=www.leigod.com?t=79">
                            </a>
                        </li>
                    </ul>
                </div>
...
<script lang="ts">
    ...
    export default class FootNav extends Vue {
        ...
        public picServerBase:string=''
        public newsList:Array<NewModel>=[]
       async created(){
           this.picServerBase=GlobalConfig.getImgBaseUrl();
           this.setBaseUrl(GlobalConfig.getBaseUrl());
           //  this.picServerBase="https://picture.leigod.com"
           // this.setBaseUrl('https://webapi.leigod.com');
           await this.getDownloadUrl();
           await this.getFriendLinks()
           await this.getNewslist()
           this.$nextTick(()=>{
               let footerHeight=document.getElementById('llz_leishen_footer').clientHeight;
               this.$emit('change-height',footerHeight)
           })
        }
        ...
        async getNewslist(){
            ...
            let params=new NewRequestModel();
            params.project=1;
            params.region_code= this.webParam.region_code;
            params.page=1;
            params.support_type=1//pc官网
            params.size = 4;
            params.class_type = 2;
            params.label=this.ztLabel
            let data=await this.http.get<AdModel>(url, params);
            this.newsList=data.data.list;
            this.$nextTick(()=>{
                $('.footer_llz_newsDesc').each(function(){
                    //设置显示获取字符串的字数 这个根绝要求 看需要大概显示几行
                    let maxwidth=40;
                    if($(this).text().length>maxwidth){
                        //截取字符串
                        $(this).text($(this).text().substring(0,maxwidth)); //多余的用省略号显示
                        $(this).html($(this).html()+'...');
                    }
                })
            })
       ...
       async getFriendLinks(){
        ...
        goToNews(item:any){
           let newUrl=GlobalConfig.getWebBaseUrl()+'/news/'+item.id+'.html'
